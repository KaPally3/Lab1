...
Вторая колонка обозначает сколько всего в системе SWAP.
...
В данном случае, количество доступного SWAP отображается в последней колонке, в случае, если SWAP используется, то колонок будет больше, но доступный объем будет все так же в последней колонке.
...
`ls /sys/class/net | cat -n`

![count](../images/7_1.png)

Выводим количество сетевых интерфейсов и их название с помощью двух утилит: `ls` и `cat` с флагом `-n`
